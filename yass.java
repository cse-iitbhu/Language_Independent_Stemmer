...
public class Main {
    ...
    private static void saveGraphData(String expName, String distanceName, List<MergeHistoryRecord> mergeHistory) {
        System.out.println("Save"+ distanceName+"...");
    ...
    private static void saveMergeHistory(String expName, String distanceName, List<MergeHistoryRecord> mergeHistory){
        System.out.println("Save data "+ distanceName+"...");
    ...
    private static List<MergeHistoryRecord> loadMergeHistory(String expName, String distanceName) {
        System.out.println("history data"+distanceName+"...");
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 2){
            ...
            System.out.println("specification fie");
            System.out.println("");
            System.out.println("$ java -jar java-IR.jar -f <experiment_file_path>");
            return;
        }
        int cores = Runtime.getRuntime().availableProcessors();
        System.out.println("Numer display: "+cores);

        String propertiesFilePath = args[1];
        System.out.println("experiments: "+ propertiesFilePath);
        ...
        System.out.println(" lexicon "+lexicon.size()+" terminal: " + (System.currentTimeMillis() - startTime)/1000 + " s");
        System.out.println("Start "+discardedNumbers + " numerical "+discardedStopwords +" stopwords");

        // Riduco il lexicon
        if (exp.getLexiconRangeStart() != 1 && exp.getLexiconRangeEnd() != 109002) {
        ...
        if (exp.getPipeline().contains(PIPE_CLUSTERING)) {
            System.out.println("clustering "+exp.getDistanceMeasure().getName());
            ...
            System.out.println("Complete clustering: " + (System.currentTimeMillis() - startTime)/1000);
            ...
            if (exp.getPipeline().contains(PIPE_SAVE_HISTORY)) {
                System.out.println("merge the data...");
        ...
        if (exp.getPipeline().contains(PIPE_MERGE_HISTORY)) {
            System.out.println("merge history...");
        ...
        System.out.println("computational time: "+(System.currentTimeMillis() - startTime)/1000+ " s");
    }
}
